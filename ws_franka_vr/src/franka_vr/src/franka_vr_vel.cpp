  ...
  auto pose_tracker = [&]() {
    ...
    while (rclcpp::ok())
    {
      if (target_set)
      {
        ...
        // ==== 安全限速的 P 控制：无论误差多大，速度都有硬上限，避免冲机触发 reflex ====
        const double kp_lin = 3.0;       // 线速度比例增益
        const double v_lin_max = 0.08;   // 最大线速度 [m/s]（标定阶段用小值; 对齐好再调大）
        double lin_speed = std::min(kp_lin * pos_error.norm(), v_lin_max);
        Eigen::Vector3d linear_vel = Eigen::Vector3d::Zero();
        if (pos_error.norm() > 1e-6)
            linear_vel = pos_error.normalized() * lin_speed;
        ...
        const double kp_ang = 1.0;       // 角速度比例增益
        const double w_ang_max = 0.3;    // 最大角速度 [rad/s]
        double ang_speed = std::min(kp_ang * angular_vel_magnitude, w_ang_max);
        Eigen::Vector3d angular_vel = Eigen::Vector3d::Zero();
        if (angular_vel_magnitude > 1e-6)
            angular_vel = Eigen::Vector3d(angular_vel_axis.x(), angular_vel_axis.y(), angular_vel_axis.z()) * ang_speed;
