...
在这之前，能跑的遥操代码活在一个没有版本控制的 Docker 卷里；容器环境只存在于"某人手工
`apt install` 过的那些包"中，没有任何文件记录；调优知识散落在各处笔记里。
几十轮真机迭代的成果，离一次 `rm -rf` 只有一步之遥。

现在复现这条通路所需的一切都在这里：

- **源码** —— 遥操包本体，加上全部第三方工作区，版本已锁定并随仓库分发
- **环境** —— 一份 Dockerfile，还原出真机上实际跑过的那个容器
- **知识** —— 结构说明、排障速查，以及一份完整的控制调优复盘
